...
public interface PostRepository extends JpaRepository<Post, Long> {
  ...
  @Query(
      "SELECT p FROM Post p LEFT JOIN FETCH p.user WHERE p.postStatus = :status ORDER BY p.postTime DESC")
  ...
  @Query(
      value =
          "SELECT EXTRACT(HOUR FROM post_time) as hour, COUNT(*) as count "
              + "FROM post "
  ...
  @Query(
      "SELECT p FROM Post p LEFT JOIN FETCH p.user u WHERE LOWER(u.username) LIKE LOWER(CONCAT('%', :username, '%'))")
