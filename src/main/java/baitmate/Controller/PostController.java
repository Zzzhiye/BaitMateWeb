...
public class PostController {
  ...
  public ResponseEntity<PostDto> getPostById(
      @PathVariable Long postId, @RequestParam(required = false) Long userId) {
  ...
  public ResponseEntity<PostDto> toggleSave(@PathVariable Long postId, @RequestParam Long userId) {
    try {
  ...
  public ResponseEntity<?> getPostsByDateRange(
    ...
    try {
      List<Post> posts =
          postService.findByPostTimeBetween(
  ...
  public ResponseEntity<?> getAllPosts(
    ...
    try {
      ...
      if (status == null
          ...
          && page == 0
          && size == 10
          && (sort == null || sort.length == 0)) { // Changed this condition
      ...
      Page<Post> postPage =
          postService.searchPostByFilter(
              status != null && !status.isEmpty() ? status : null, pageable);
