...
public class SavedLocationController {
  ...
  @Autowired FishingLocationService fishingLocationService;
  @Autowired private UserRepository userRepository;

  @PostMapping("/remove")
  public ResponseEntity<?> removeSavedLocation(
      @RequestParam Long userId, @RequestParam Long locationId) {
  ...
  @PostMapping("/save")
  public ResponseEntity<?> saveFishingLocation(
      @RequestParam Long userId, @RequestParam Long locationId) {
    User user = userService.searchByUserId(userId);
    FishingLocation location = fishingLocationService.getFishingSpotById(locationId).orElse(null);
    if (user == null || location == null) {
