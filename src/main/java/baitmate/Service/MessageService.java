...
public class MessageService {
  ...
  public void sendAnnouncement(MessageDTO messageDTO, MultipartFile[] attachments) {
    ...
    try {
      ...
      message =
          messageRepository.save(message); // Save the message *before* sending emails/notifications
        ...
        if (file != null && !file.isEmpty()) { // Check for null and emptiness
          ...
          if (originalFilename == null) {
            throw new IllegalArgumentException(
                "Original filename is null"); // Or handle it more gracefully
          }
  ...
  private void sendToAllUsers(MessageDTO messageDTO, List<String> savedFileNames) {
    ...
    for (User user : allUsers) {
      createNotificationAndSendEmail(
          user, messageDTO.getSubject(), messageDTO.getText(), savedFileNames);
    }
  }
  ...
  private void sendToSingleUser(MessageDTO messageDTO, List<String> savedFileNames) {
    User user =
        userRepository
            .findByUsername(messageDTO.getRecipient())
            .orElseThrow(
                () -> new RuntimeException("User not found: " + messageDTO.getRecipient()));
    createNotificationAndSendEmail(
        user, messageDTO.getSubject(), messageDTO.getText(), savedFileNames);
  }

  private void createNotificationAndSendEmail(
      User user, String subject, String text, List<String> attachmentFileNames) {
    // Save notification
    ...
    if (user.getEmail() != null && !user.getEmail().isEmpty()) {
      try {
        if (!attachmentFileNames.isEmpty()) {
          ...
          // 过滤掉任何null值
          multipartFiles =
              Arrays.stream(multipartFiles).filter(Objects::nonNull).toArray(MultipartFile[]::new);
