...
public class PostReportServiceImpl implements PostReportService {
    ...
    // Filtering Method
    public List<Post> getFilteredPosts(List<Long> postIds, boolean allPosts,
    ...
    public ByteArrayInputStream generateWordReport(List<Long> postIds, boolean allPosts,
    ...
    public void addSummarySection(XWPFDocument document, List<Post> posts) {
        ...
        summaryRun.addBreak();
        summaryRun.setText("Active Posts: " +
                posts.stream().filter(p -> "active".equalsIgnoreCase(p.getPostStatus())).count());
        summaryRun.addBreak();
        summaryRun.setText("Total Likes: " +
                posts.stream().mapToInt(Post::getLikeCount).sum());
        summaryRun.addBreak();
        summaryRun.setText("Total Saves: " +
                posts.stream().mapToInt(Post::getSavedCount).sum());
        summaryRun.addBreak();
        summaryRun.setText("Average Accuracy Score: " +
                String.format("%.2f", posts.stream()
                        .mapToDouble(Post::getAccuracyScore)
                        .average()
                        .orElse(0.0)));
        summaryRun.addBreak();
        summaryRun.addBreak();
    ...
    public void createPostTable(XWPFDocument document, List<Post> posts) {
    ...
    public void styleHeaderCell(XWPFTableCell cell) {
    ...
    public void addEngagementStats(XWPFDocument document, List<Post> posts) {
        ...
        List<Post> topLikedPosts = posts.stream()
        ...
        for (Post post : topLikedPosts) {
            engagementRun.setText(String.format("- %s (ID: %d) - %d likes",
                    post.getPostTitle(), post.getId(), post.getLikeCount()));
            engagementRun.addBreak();
        }
    }

    public void addLocationAnalysis(XWPFDocument document, List<Post> posts) {
    ...
    public ByteArrayInputStream generateExcelReport(List<Long> postIds, boolean allPosts,
                                                    LocalDate startDate, LocalDate endDate,
                                                    boolean includePosts) throws IOException {
        List<Post> posts = getFilteredPosts(postIds, allPosts, startDate, endDate, includePosts);
    ...
    public void createPostsSheet(Sheet sheet, List<Post> posts) {
    ...
    public void createSummarySheet(Sheet sheet, List<Post> posts) {
        CellStyle headerStyle = createHeaderStyle(sheet.getWorkbook());
        ...
        addSummaryRow(sheet, rowNum++, "Active Posts",
                posts.stream().filter(p -> "active".equalsIgnoreCase(p.getPostStatus())).count(), boldStyle);
        addSummaryRow(sheet, rowNum++, "Total Likes",
                posts.stream().mapToInt(Post::getLikeCount).sum(), boldStyle);
        addSummaryRow(sheet, rowNum++, "Total Saves",
                posts.stream().mapToInt(Post::getSavedCount).sum(), boldStyle);
        addSummaryRow(sheet, rowNum++, "Average Accuracy Score",
                posts.stream().mapToDouble(Post::getAccuracyScore).average().orElse(0.0), boldStyle);
    }

    public void createLocationSheet(Sheet sheet, List<Post> posts) {
        CellStyle headerStyle = createHeaderStyle(sheet.getWorkbook());
        ...
        Map<String, List<Post>> locationGroups = posts.stream()
                .collect(Collectors.groupingBy(Post::getLocation));
        ...
        for (Map.Entry<String, List<Post>> entry : locationGroups.entrySet()) {
            ...
            row.createCell(2).setCellValue(locationPosts.stream()
                    .mapToInt(Post::getLikeCount).sum());
            row.createCell(3).setCellValue(locationPosts.stream()
                    .mapToDouble(Post::getAccuracyScore)
                    .average().orElse(0.0));
        }
    }
    ...
    public CellStyle createBoldStyle(Workbook workbook) {
        CellStyle style = workbook.createCellStyle();
        Font font = workbook.createFont();
        font.setBold(true);
        style.setFont(font);
    ...
    public void addSummaryRow(Sheet sheet, int rowNum, String label, Number value, CellStyle style) {
        Row row = sheet.createRow(rowNum);
        ...
        if (value instanceof Double) {
            valueCell.setCellValue(((Double) value).doubleValue());
        } else {
            valueCell.setCellValue(value.longValue());
        }
    }

    public void autoSizeAllSheets(Workbook workbook) {
